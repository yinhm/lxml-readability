/* 
$Id: altClickToSearch.js 72751 2011-07-25 18:42:06Z helckt $
(c)2009 - 2011 The New York Times Company  
*/
...
NYTD.WordReference = (function(){
	...
	if (Prototype.Browser.IE) {
		var ie_version = parseInt(navigator.userAgent.match(/MSIE\s([\d+])/)[1], 10);
		var supportsStandardSelections = ie_version > 8;
	}

	var buttons = {
		"wordReference": {
			"mouseupHandler": exportSelection,
			"isEligible" : function(wordCount) { return (wordCount < 4 && wordCount > 0); },
			"element": new Element(
				'span', {
				...
				buttonsHash.keys().each( function(name) {
					...
					/* Temporary blogger test code */
						...
						if(buttons["timesQuotes"].isEligible(wc)) {
							...
							var hostName = window.location.href.toString().match(/.*\.com/);
	...
	function insertButton(buttonName) {
		...
		if(buttons[buttonName].isEligible(wc)) {
			...
			if (Prototype.Browser.IE && !supportsStandardSelections) {
